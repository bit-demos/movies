...
The factory connects to the `useApiContext` of any api context that is based on the [rest-ful API context provider component](@learn-bit-react/movies.context.api-context-provider),
...
1. `processData` - the function which will be used to process the raw api data to produce the desired output data from the hook
